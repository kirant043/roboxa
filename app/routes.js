...
module.exports = function (app) {
  ...
  app.get('/api/client', function (req, res) {
    ...
    var query = {};
    dbaa.collection("client").find(query).toArray(function (err, result) {
      if (err) throw err;
      // console.log(result);
  ...
  app.post('/api/userdetails', function (req, res) {
    ...
    dbaa.collection("user").find(query).toArray(function (err, result) {
      if (err) throw err;
      // // console.log(result);
  ...
  app.post('/api/mobilelogin', function (req, res) {
    ...
    dbaa.collection("supervisor").find(login).toArray(function (err, result) {
      if (err) throw err;
      ...
      if (result.length > 0) {
        ...
        var pickdate = new Date().toLocaleString('en-US', {
          timeZone: 'Asia/Kuala_Lumpur'
        }).split(",")[0].split("/");
        var picktime = new Date().toLocaleString('en-US', {
          timeZone: 'Asia/Kuala_Lumpur'
        }).split(",")[1]
        var token_data = {
        ...
        dbaa.collection("usertokkendetails").remove({
          ...
          dbaa.collection("usertokkendetails").insert(token_data, function (err, result) {
            if (err) throw err;
  ...
  app.post('/api/savenotfication', function (req, res) {
    ...
    dbaa.collection("notificationdetail").insert(req.body, function (err, result) {
      if (err) throw err;
  ...
  app.post('/api/findtoken', function (req, res) {
    ...
    dbaa.collection("usertokkendetails").find(req.body).toArray(function (err, result) {
      if (err) throw err;
      // // console.log(result);
  ...
  app.post('/api/listNotification', function (req, res) {
    ...
    dbaa.collection("notificationdetail").find({
      "supervisor_id": req.body.supervisor_id
    }).sort([
      ['_id', -1]
  ...
  app.post('/api/showdatadoc', function (req, res) {
    ...
    dbaa.collection("alldocuments").find(query).toArray(function (err, result) {
      if (err) throw err;
      // // console.log(result);
  ...
  app.post('/api/callhistorydata', function (req, res) {
    ...
    dbaa.collection("callhistory").find(req.body).toArray(function (err, result) {
      if (err) throw err;
      // // console.log(result);
...
if(req.body.user_id!=''){
 datsend={ $or : [ { user_id : req.body.user_id }, { other_user_id : req.body.user_id } ] } 
}
     

    dbaa.collection("callhistory").find(datsend).toArray(function (err, result) {
      if (err) throw err;
      // // console.log(result);
  ...
  app.post('/api/viewevidence', function (req, res) {
    ...
    dbaa.collection("evidence").find(req.body).toArray(function (err, result) {
      if (err) throw err;
  ...
  app.post('/api/saveglasswisedoclist', function (req, res) {
    ...
    dbaa.collection("glasswisedoclist").insert(req.body, function (err, result) {
      if (err) throw err;
  ...
  app.post('/api/getglasswisedoclist', function (req, res) {
    ...
    dbaa.collection("glasswisedoclist").aggregate([{
        $unwind: "$doclist"
      },
      {
        $match: req.body
  ...
  app.post('/api/login', function (req, res) {
    ...
    dbaa.collection("user").find(datadsds).toArray(function (err, result) {
      if (err) throw err;
      if (result.length > 0) {
        result = result[0];
        var returnData = {
        ...
        if (result.supervisor_id !== null && result.supervisor_id !== undefined && result.supervisor_id !== '') {
  ...
  app.get('/view/supervisor', function (req, res) {
    ...
    if (req.query.access_role == 'admin') {
      dbaa.collection("supervisor").find().toArray(function (err, result) {
        if (err) throw (err);
        if (result) {
          res.json(result);
        } else {
  ...
  app.get('/view/worker', function (req, res) {
    ...
    if (req.query.access_role == "admin") {
      dbaa.collection("user").find().toArray(function (err, result) {
        if (err) throw (err);
        if (result) {
          res.json(result);
        } else {
  ...
  app.post('/api/nocallworkers', function (req, res) {
    dbaa.collection("traingevidence").aggregate([{
      $match: {
        supervisor_id: req.body.supervisor_id,
      }
    },{
      ...
      $replaceRoot: { newRoot: { $mergeObjects: [ { $arrayElemAt: [ "$list", 0 ] }, "$$ROOT" ] } }
    },{ 
      $project: { list: 0 }
    },{
      $sort: { "_id":1 }
  ...
  app.post('/api/recordedworkers', function (req, res) {
    dbaa.collection("callrecordings").aggregate([{
      $match: {
        supervisor_id: req.body.supervisor_id,
      }
    },{
