...
module.exports = function (app) {
  ...
  var user = []; // 3 login places 1 4 mobile the others i don't know which 1 u use to login 
  app.post('/api/userlogin',loginFunction);
  app.post("/api/userlogin1", function (req, res) { // try
    ...
    dbaa
      ...
      .toArray(function (err, result) {
        ...
        if (result.length > 0) {
          ...
          dbaa.collection("usertokkendetails").remove(
            {
              deviceID: req.body.deviceID, // try to login from postman ok
            },
            function (err, result) {
              if (err) throw err;
              // console.log("remove detaisl",result);
              dbaa
                ...
                .insert(token_data, function (err, result) {
                  ...
                  var response = {
                    supervisorData: {
                      full_name: findata.f_name + " " + findata.l_name,
                      id: supervisor_idtoken,'xs':'xs' // try
                    },
                    msg: "login Successfully",
                    status: "200",
                  };
...
app.post("/api/login",loginFunction);
  //app.post("/api/login", function (req, res) { //try
    console.log(req.body);
